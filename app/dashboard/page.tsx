      ...
      <div className="p-8 max-w-6xl mx-auto space-y-8 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          <div className="bg-bg-card p-6 rounded-xl border border-bg-input shadow-lg flex flex-col">
            <span className="text-text-muted text-sm uppercase font-semibold mb-1">
              Total ASINs
            </span>
            <span className="text-2xl font-extrabold text-white">
              {overall.total_asins.toLocaleString()}
            </span>
          </div>
          <div className="bg-bg-card p-6 rounded-xl border border-bg-input shadow-lg flex flex-col">
            <span className="text-text-muted text-sm uppercase font-semibold mb-1">
            ...
            <span className="text-2xl font-extrabold text-accent">
              {overall.total_attributes.toLocaleString()}
            </span>
          </div>
          <div className="bg-bg-card p-6 rounded-xl border border-bg-input shadow-lg flex flex-col">
            <span className="text-text-muted text-sm uppercase font-semibold mb-1">
          ...
          <div className="bg-bg-card p-6 rounded-xl border border-purple-500/30 bg-gradient-to-br from-bg-card to-purple-900/10 shadow-lg flex flex-col relative overflow-hidden">
            ...
            <span className="text-text-muted text-sm uppercase font-semibold mb-1 z-10 text-purple-300">
              Tavily Credits Burned
            </span>
            <span className="text-2xl font-extrabold text-purple-400 z-10">
              {overall.total_tavily_credits?.toLocaleString() || 0}
            </span>
          </div>
          <div className="bg-bg-card p-6 rounded-xl border border-bg-input shadow-lg flex flex-col">
            <span className="text-text-muted text-sm uppercase font-semibold mb-1">
            ...
            <span className="text-2xl font-extrabold text-gray-400">
              {overall.total_sessions.toLocaleString()}
            </span>
          </div>
        </div>
        ...
        <div className="bg-bg-card p-6 rounded-xl border border-bg-input shadow-lg overflow-hidden">
          ...
          {recent_sessions.length > 0 ? (
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-bg-input">
                  {recent_sessions.map((sess: any) => (
                    <tr
                      key={sess.session_id}
                      className="hover:bg-bg-input/30 transition-colors"
                    >
            ...
            <div className="p-8 text-center text-text-muted border border-dashed border-bg-input rounded-lg">
              No recent sessions found. Go to Attribute Master to start
              processing!
            </div>
          )}
        </div>
