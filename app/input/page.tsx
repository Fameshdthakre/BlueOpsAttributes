    ...
    try {
      ...
      if (validationFile && valDataStr) {
        ...
        valData.forEach((row: any) => {
          ...
          const pType = valPtypeCol ? (row[valPtypeCol]?.toString().trim() || "") : "";
          const vals = row[valDdCol]?.toString().split(',').map((v: string) => v.trim()).filter(Boolean) || [];
  ...
  return (
    <div className="animate-in fade-in flex flex-col h-full">
      <PageHeader 
        ...
        breadcrumbs={[{ label: 'BlueOps Hub', href: '/' }, { label: 'Attribute Master' }]}
      >
        <a 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="bg-bg-dark rounded-lg overflow-x-auto w-full border border-bg-input">
                <table className="w-full text-left text-sm whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-bg-input">
                    ...
                    <tr>
                      <td className="p-3 text-text-main font-medium flex items-center gap-2">Attribute ID <span className="text-status-error text-xs">*</span></td>
                      <td className="p-2">
                    ...
                    <tr>
                      <td className="p-3 text-text-main font-medium">Product Type</td>
                      <td className="p-2">
                    ...
                    <tr>
                      <td className="p-3 text-text-main font-medium">Brand</td>
                      <td className="p-2">
                    ...
                    <tr>
                      <td className="p-3 text-text-main font-medium">Title</td>
                      <td className="p-2">
        ...
        <div className="bg-bg-card p-6 rounded-xl border border-bg-input" id="tour-validation-upload">
          <h2 className="text-xl font-semibold mb-4 text-text-main flex items-center gap-2">
            <span className="text-primary">2</span> Validation File <span className="text-sm font-normal text-text-muted">(Optional)</span>
          </h2>
          ...
          <input 
            ...
            className="block w-full text-sm text-text-muted
              ...
              file:rounded-md file:border-0
              file:text-sm file:font-semibold
              file:bg-surface-2 file:text-text-main
              hover:file:bg-bg-input
              cursor-pointer mb-6"
          />
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-bg-input">
                    <tr>
                      <td className="p-3 text-text-main font-medium flex items-center gap-2">Attribute ID <span className="text-status-error text-xs">*</span></td>
                      <td className="p-2">
                        <select value={valAttrCol} onChange={e => setValMappings({...valMappings, valAttrCol: e.target.value})} className="w-full bg-bg-card border-none rounded p-2 text-text-main outline-none focus:ring-1 focus:ring-primary transition-all">
                          <option value="">-- Select --</option>
                    ...
                    <tr>
                      <td className="p-3 text-text-main font-medium">Product Type</td>
                      <td className="p-2">
                        <select value={valPtypeCol} onChange={e => setValMappings({...valMappings, valPtypeCol: e.target.value})} className="w-full bg-bg-card border-none rounded p-2 text-text-main outline-none focus:ring-1 focus:ring-primary transition-all">
                          <option value="">-- Optional --</option>
                          {validationHeaders.map(h => <option key={h} value={h}>{h}</option>)}
                        </select>
                      </td>
                    ...
                    <tr>
                      <td className="p-3 text-text-main font-medium flex items-center gap-2">Allowed Values <span className="text-status-error text-xs">*</span></td>
                      <td className="p-2">
                        <select value={valDdCol} onChange={e => setValMappings({...valMappings, valDdCol: e.target.value})} className="w-full bg-bg-card border-none rounded p-2 text-text-main outline-none focus:ring-1 focus:ring-primary transition-all">
                          <option value="">-- Select --</option>
                          {validationHeaders.map(h => <option key={h} value={h}>{h}</option>)}
                        </select>
                      </td>
      ...
      {totalJobsCount > 0 && (
        <div id="processing-section" className="space-y-8 pt-8 border-t border-bg-input mt-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
            <h2 className="text-2xl font-bold text-text-main flex items-center gap-2">
              <span className="text-primary">3</span> Processing Dashboard <span className="text-sm font-normal text-text-muted">({totalJobsCount} Jobs)</span>
            </h2>
            
            <div className="flex gap-2 w-full md:w-auto">
              {!running ? (
                <button id="tour-start-process" onClick={startProcessing} className="flex-1 md:flex-none text-sm bg-primary hover:bg-primary-hover text-white px-6 py-2.5 rounded-lg font-bold shadow-lg shadow-primary/20 flex items-center justify-center gap-2 transition-all transform hover:scale-105">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" /></svg>
                  {processedCount > 0 ? "Resume" : "Start Extraction"}
                </button>
              ) : (
                <>
                  {paused ? (
                    <button onClick={resumeProcessing} className="flex-1 md:flex-none text-sm bg-primary hover:bg-primary-hover text-white px-6 py-2.5 rounded-lg font-bold flex items-center justify-center gap-2 transition-all">
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" /></svg>
                      Resume
                    </button>
                  ) : (
                    <button onClick={pauseProcessing} className="flex-1 md:flex-none text-sm bg-status-warning hover:bg-status-warning/80 text-bg-dark px-6 py-2.5 rounded-lg font-bold flex items-center justify-center gap-2 transition-all">
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zM7 8a1 1 0 012 0v4a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v4a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
                      Pause
                    </button>
                  )}
                  <button onClick={cancelProcessing} className="flex-1 md:flex-none text-sm bg-status-error/10 hover:bg-status-error text-status-error hover:text-white border border-status-error/30 hover:border-status-error px-4 py-2.5 rounded-lg font-semibold flex items-center justify-center gap-2 transition-colors">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8 7a1 1 0 00-1 1v4a1 1 0 001 1h4a1 1 0 001-1V8a1 1 0 00-1-1H8z" clipRule="evenodd" /></svg>
                    Cancel
                  </button>
                </>
              )}
            ...
            <div className="md:col-span-2 bg-bg-card p-6 rounded-xl border border-bg-input">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-bg-dark border border-status-success/20 p-4 rounded-lg text-center">
                  <div className="text-3xl font-bold text-status-success">{validatedCount}</div>
                  <div className="text-xs text-text-muted uppercase mt-1 tracking-wide font-medium">Validated</div>
                </div>
                <div className="bg-bg-dark border border-status-warning/20 p-4 rounded-lg text-center">
                  <div className="text-3xl font-bold text-status-warning">{unresolvedCount}</div>
                  <div className="text-xs text-text-muted uppercase mt-1 tracking-wide font-medium">Unresolved</div>
                </div>
                <div className="bg-bg-dark border border-status-error/20 p-4 rounded-lg text-center">
                  <div className="text-3xl font-bold text-status-error">{failedCount}</div>
                  <div className="text-xs text-text-muted uppercase mt-1 tracking-wide font-medium">Failed</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#090C10] rounded-xl border border-bg-input shadow-inner flex flex-col h-[500px]">
            ...
            <div className="p-3 border-b border-bg-input flex flex-col md:flex-row justify-between items-center gap-4 bg-bg-dark rounded-t-xl">
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  <svg className="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                  Download
                </button>
              </div>
            </div>
