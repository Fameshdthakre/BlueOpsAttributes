...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Actual Sidebar */}
      <div
        className={`fixed md:static inset-y-0 left-0 z-40 h-full bg-bg-card border-bg-input flex flex-col transition-all duration-300 ease-in-out
      ...
      >
        ...
        <Link
          ...
          className={`flex items-center gap-3 px-4 py-5 border-b border-bg-input transition-colors ${
        ...
        >
          ...
          <span
            className={`font-bold text-base whitespace-nowrap transition-opacity duration-300 ${isExpanded ? "opacity-100" : "opacity-0 w-0 overflow-hidden"}`}
          >
            Dashboard
          </span>
        </Link>
                ...
                group.items.map((item) => {
                  const isActive =
                    item.href === "/attribute-master"
                      ? pathname.startsWith("/attribute-master") ||
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`text-sm whitespace-nowrap transition-opacity duration-200 ${isExpanded ? "opacity-100" : "opacity-0 w-0 overflow-hidden"}`}
                      >
                        {item.name}
                      </span>
        ...
        {/* Footer */}
        <div
          className={`p-4 border-t border-bg-input transition-opacity duration-300 ${isExpanded ? "opacity-100" : "opacity-0 h-0 overflow-hidden py-0 border-transparent"}`}
        >
          <div className="text-xs text-text-muted text-center">
            BlueOps Enterprise v2.0
          </div>
