...
const KeyVaultSection = ({
  ...
  return (
    <div className="bg-bg-card p-6 rounded-xl border border-bg-input shadow-sm">
      <h2 className="text-xl font-semibold text-text-main flex items-center gap-2 mb-6 pb-4 border-b border-bg-input">
        <span className={`w-3 h-3 rounded-full ${iconColor}`}></span>
        {providerName} API Keys
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          <h3 className="text-sm font-semibold text-text-muted mb-3 uppercase tracking-wider flex justify-between items-center">
            Active Keys
            <span className="bg-bg-input text-text-main px-2 py-0.5 rounded-full text-xs">{keys.length}</span>
          </h3>
          {keys.length === 0 ? (
            <div className="text-text-muted text-sm italic bg-bg-dark border border-dashed border-bg-input rounded-lg p-6 text-center flex flex-col items-center gap-2">
              <svg className="w-8 h-8 text-bg-input" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
              No keys configured. Add some via bulk import.
            </div>
          ) : (
            <ul className="space-y-2 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar">
              {keys.map((k, idx) => (
                <li key={idx} className="flex justify-between items-center bg-bg-dark border border-bg-input rounded px-3 py-2.5">
                  <span className="font-mono text-sm text-text-main opacity-80">
                    {k.length > 15 ? k.substring(0, 5) + "••••••••" + k.slice(-4) : "••••••••"}
                  </span>
                  <button onClick={() => handleDelete(idx)} className="text-status-error hover:bg-status-error/10 p-1.5 rounded transition-colors" title="Delete Key">
        ...
        <div>
           <h3 className="text-sm font-semibold text-text-muted mb-3 uppercase tracking-wider">Bulk Import</h3>
           <textarea 
             value={bulkInput}
             onChange={e => setBulkInput(e.target.value)}
             className="w-full bg-bg-dark border border-bg-input rounded p-3 text-sm text-text-main focus:border-primary focus:ring-1 focus:ring-primary outline-none h-[120px] resize-none"
             placeholder="Paste multiple API keys here (separated by newlines or commas)..."
           />
           <button onClick={handleBulkAdd} disabled={!bulkInput.trim()} className="mt-3 w-full bg-bg-input hover:bg-primary text-white font-semibold py-2.5 rounded-lg transition-colors disabled:opacity-50 disabled:hover:bg-bg-input">
             Import Keys
           </button>
        </div>
      </div>
...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"general" | "keyvault" | "personal">("general");
      ...
      <div className="p-8 max-w-5xl mx-auto space-y-8 overflow-y-auto flex-1 pb-20 w-full">
        ...
        {activeTab === "keyvault" && (
          <div className="space-y-6">
            <div className="bg-bg-card p-6 rounded-xl border border-primary/20 mb-6 bg-gradient-to-r from-bg-card to-primary/5">
              ...
              <p className="text-sm text-text-muted">
                Manage all your API keys securely in one place. Keys are automatically rotated when quota errors are hit.
              </p>
            </div>
            {PROVIDERS.map((provider) => (
              <KeyVaultSection
                ...
                iconColor={provider === "Gemini" ? "bg-blue-500" : provider === "OpenAI" ? "bg-green-500" : "bg-orange-500"}
                keysStr={config.providers[provider].api_key || ""}
                onChange={(val) =>
                  updateConfig({
                    ...config,
                    providers: {
                      ...config.providers,
                      [provider]: { ...config.providers[provider], api_key: val },
                    },
                  })
                }
              />
            ...
            <KeyVaultSection
              ...
              keysStr={config.providers.Tavily?.api_key || ""}
              onChange={(val) =>
                updateConfig({
                  ...config,
                  providers: {
                    ...config.providers,
                    Tavily: { ...config.providers.Tavily, api_key: val },
                  },
                })
              }
            />
